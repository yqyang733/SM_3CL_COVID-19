...
def col_pic(file):
    ...
    plt.bar(x,y1,width=0.2,label='I3C-0',edgecolor='#C35C6A', linewidth=0,color="#66889E")
    # plt.bar(x+0.3,y2,width=0.3,color='#FC4E07',label='20ns',edgecolor='#FC4E07', linewidth=0.25)                 #调整y2轴位置，颜色，label为图例名称，与下方legend结合使用
    plt.bar(x+0.2,y2,width=0.2,label='I3C-1',edgecolor='#66889E', linewidth=0,color="#C35C6A")
    plt.bar(x+0.4,y3,width=0.2,label='I3C-2',edgecolor='#7E527F', linewidth=0,color="#7E527F")
    ...
    plt.legend(loc=(0.4,0.92),ncol=3,frameon=False,prop="Arial")    #显示图例，loc图例显示位置(可以用坐标方法显示），ncol图例显示几列，默认为1列,frameon设置图形边框
