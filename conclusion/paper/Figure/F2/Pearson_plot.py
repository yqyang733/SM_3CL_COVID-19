...
def col_pic(file):
    ...
    plt.bar(x[0],y1[0],width=0.45,color='#91341D',label='gbsa',edgecolor='k', linewidth=0, alpha=1) # yerr=er_1, error_kw=error_attri_1, alpha=1)                     #调整y1轴位置，颜色，label为图例名称，与下方legend结合使用
    plt.bar(x[1],y1[1],width=0.45,color='#C65323',label='gbsa',edgecolor='k', linewidth=0, alpha=1)
    plt.bar(x[2],y1[2],width=0.45,color='#E96B3D',label='gbsa',edgecolor='k', linewidth=0, alpha=1)
    plt.bar(x[3],y1[3],width=0.45,color='#F9885E',label='gbsa',edgecolor='k', linewidth=0, alpha=1)
    plt.bar(x[4],y1[4],width=0.45,color='#FAB590',label='gbsa',edgecolor='k', linewidth=0, alpha=1)
    ...
    plt.ylabel('Pearson Coefficient',fontproperties="Arial",fontsize=24,weight="bold")
